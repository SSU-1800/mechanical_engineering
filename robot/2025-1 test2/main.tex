...
\author{
	...
	시험 실시 : 2025-06-10 18:30-20:30+$\alpha$\;(120분+$\alpha$)\\[110pt]
	}
\date{\today}
...
\begin{align*}
	&\overline{BP} = \frac{0.2\meter}{\sin 45\dgr}\cdot \sin 25\dgr = 119.5345\mm\\
	&\overline{AP} = \frac{0.2\meter}{\sin 45\dgr}\cdot \sin 110\dgr = 265.785\mm\\
	&\bm{\omega} = 5\mathbf{k}\snd^{-1},\quad \dot{\bm{\omega}} = -2\mathbf{k}\snd^{-2}\\
	&\mathbf{v}_P = \bm{\omega}\times\mathbf{r}_{P/A} = 1.328926\mps\measuredangle 155\dgr\\
	&v_{P,\theta} = v_P \cos(155\dgr - 110\dgr) = 0.939693\mps\\
	&\omega_{BD} = \frac{v_{P,\theta}}{r_{P/B}} = 7.86127\snd^{-1}(\circlearrowleft)\\
	&\mathbf{a}_P = \bm{\omega}\times(\bm{\omega}\times \mathbf{r}_{P/A}) + \dot{\bm{\omega}}\times\mathbf{r}_{P/A} = 6.64463\mpss\measuredangle (-115\dgr) + 0.531570\mpss\measuredangle155\dgr\\
	&\quad = 4.32259\mpss\measuredangle(-70\dgr) + 5.07434\mpss\measuredangle(-160\dgr)\\
	&a_{P,\theta} = 4.32259\mpss\\
	&\dot{\omega}_{BD} = \frac{a_{P,\theta}}{r_{P/B}} = 16.26346\snd^{-2}(\circlearrowright)\\
	&\ddot{\mathbf{r}}_D = \bm{\omega}_{BD}\times(\bm{\omega}_{BD}\times \mathbf{r}_{D/B}) + \dot{\bm{\omega}}_{BD}\times\mathbf{r}_{D/B} = 61.7995\mpss\measuredangle(-160\dgr) + 16.26346\mpss\measuredangle(-70\dgr)\\
	&\quad = 63.904\mpss\measuredangle(-174.744\dgr)
\end{align*}
\asw{}{$\mathbf{a}_D = 63.904\mpss\measuredangle(-174.744\dgr)$}
...
\probpic{Question 4 | Prob. 15.171 in textbook}{img/q04.png}{60mm}{
	...
	*femur : 대퇴골, ankle : 발목, tibia : 정강이 뼈
	...
	\begin{align*}
		\text{Given :}\quad & l = 50\mm,\quad v_A = 0,\quad \bm{\Omega} = \bm{\omega}_{AK} = 1.5\mathbf{k}\snd^{-1},\quad \dot{\bm{\Omega}} = \dot{\bm{\omega}}_{AK} = 1\mathbf{k}\snd^{-2}\\
	...
	\end{align*}
	\begin{align*}
		&\mathbf{r}_{K/A} = l(6\mathbf{i} + 6\mathbf{j}),\quad \mathbf{r}_{H/K} = l(-7\mathbf{i} + 7\mathbf{j}),\quad \mathbf{r}_{H/A} = \mathbf{r}_{K/A} + \mathbf{r}_{H/K} = l(-1\mathbf{i} + 13\mathbf{j})\\
